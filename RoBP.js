...
(function() {
    ...
    style.textContent = '#i2d-popup { display: none !important; }';
    document.head.appendChild(style);
removeRophimAds();


    // --- Dưới đây là định nghĩa function ---
    function removeRophimAds() {
        const run = () => {
            const hideSelectors = [
                'div.sspp-area.is-wide',
                'div.sspp-area.is-square-400',
                'div.sspp-area.is-poster',
                'div.display-single.is-demo',
                'div.fade.v-modal.d-modal.sspp-modal.modal.show',
                'div.fade.modal-backdrop.show'
            ];
            document.querySelectorAll(hideSelectors.join(',')).forEach(el => el.style.display = 'none');

            document.querySelectorAll('div.swiper-slide').forEach(slide => {
                if (slide.querySelector('div.sspp-area.is-poster')) slide.style.display = 'none';
            });

            document.querySelectorAll('div.sspp-area.is-player.is-pop.ssp-pop-id').forEach(el => {
